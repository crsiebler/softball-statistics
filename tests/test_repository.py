...
from softball_statistics.models import League, Team, Player
...
class TestSQLiteRepository:
    def test_init_creates_database(self):
        ...
        with patch('sqlite3.connect') as mock_connect, \
             patch.object(SQLiteRepository, '_create_tables') as mock_create:
            mock_conn = Mock()
    ...
    def test_save_and_get_league(self):
        ...
        with patch.object(SQLiteRepository, '_create_tables'), \
             patch.object(SQLiteRepository, '_get_connection') as mock_get_conn:
            mock_conn = Mock()
            mock_cursor = Mock()
    ...
    def test_save_parsing_warnings(self):
        ...
        with patch.object(SQLiteRepository, '_create_tables'), \
            ...
            assert 'player_name, row_num, col_num, filename, original_attempt, assumption' in sql
            assert 'VALUES (?, ?, ?, ?, ?, ?)' in sql
            assert first_call[0][1] == ('John', 2, 3, 'test.csv', 'hr', 'HR solo (assumed 1 RBI, 1 run scored)')
