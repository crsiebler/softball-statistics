import pytest
from softball_statistics.parsers.attempt_parser import parse_attempt, AttemptParseError
...
class TestAttemptParser:
    def test_single_hit(self):
        ...
        assert result == {'hit_type': 'single', 'bases': 1, 'rbis': 0, 'runs_scored': 0, 'warnings': []}

    def test_double_hit(self):
        """Test parsing a double (2B)."""
        result = parse_attempt("2B")
        assert result == {'hit_type': 'double', 'bases': 2, 'rbis': 0, 'runs_scored': 0, 'warnings': []}

    def test_triple_hit(self):
        """Test parsing a triple (3B)."""
        result = parse_attempt("3B")
        assert result == {'hit_type': 'triple', 'bases': 3, 'rbis': 0, 'runs_scored': 0, 'warnings': []}

    def test_home_run_hit(self):
        """Test parsing a home run (HR)."""
        result = parse_attempt("HR")
        expected = {'hit_type': 'home_run', 'bases': 4, 'rbis': 1, 'runs_scored': 1, 'warnings': [{
            'player_name': '',
            'row_num': 0,
            'col_num': 0,
            'filename': '',
    ...
    def test_strikeout(self):
        ...
        assert result == {'hit_type': 'out', 'bases': 0, 'rbis': 0, 'runs_scored': 0, 'warnings': []}

    def test_walk(self):
        """Test parsing a walk (BB)."""
        result = parse_attempt("BB")
        assert result == {'hit_type': 'walk', 'bases': 0, 'rbis': 0, 'runs_scored': 0, 'warnings': []}
    ...
    @pytest.mark.parametrize("invalid_attempt", [
        "",      # Empty string
        "*",     # Just modifiers
        "+",     # Just modifiers
        "1C",    # Invalid hit type
    ...
    def test_hr_too_many_rbis(self):
        ...
        with pytest.raises(AttemptParseError, match="Home runs cannot have more than 4 RBIs"):
            parse_attempt("HR*****")

    def test_hr_solo_with_star(self):
        """Test HR* generates warning about solo assumption."""
        result = parse_attempt("HR*", player_name="John", row_num=2, col_num=3, filename="test.csv")
        expected = {
            'hit_type': 'home_run',
            'bases': 4,
            'rbis': 1,
    ...
    def test_whitespace_handling(self):
        ...
        result = parse_attempt(" 1B * + ")
        assert result == {'hit_type': 'single', 'bases': 1, 'rbis': 1, 'runs_scored': 1, 'warnings': []}