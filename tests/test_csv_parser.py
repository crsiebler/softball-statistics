...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='_test-team-season-01.csv', delete=False) as f:
            f.write(csv_content)
            f.flush()
            ...
            try:
                ...
                hr_attempt = next(a for a in result['attempts'] if a['attempt_raw'] == 'HR**')
                assert hr_attempt['hit_type'] == 'home_run'
                assert hr_attempt['bases'] == 4
                assert hr_attempt['rbis'] == 2
                assert hr_attempt['runs_scored'] == 0

                triple_attempt = next(a for a in result['attempts'] if a['attempt_raw'] == '3B*+')
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='_test-team-season-01.csv', delete=False) as f:
            ...
            try:
                with pytest.raises(CSVParseError, match="First column must be 'Player Name'"):
                    parse_csv_file(f.name)
            finally:
                os.unlink(f.name)
...
"""
    ...
    def test_create_database_objects(self):
        ...
        parsed_data = {
            'metadata': {'league': 'TestLeague', 'team': 'TestTeam', 'season': 'Winter', 'game': '01'},
            'player_names': ['Player1', 'Player2'],
            'attempts': [
                {'player_name': 'Player1', 'attempt_raw': '1B', 'hit_type': 'single', 'bases': 1, 'rbis': 0, 'runs_scored': 0},
                {'player_name': 'Player2', 'attempt_raw': '2B*', 'hit_type': 'double', 'bases': 2, 'rbis': 1, 'runs_scored': 0}
            ]
        }
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='_test-team-season-01.csv', delete=False) as f:
            ...
            try:
                ...
                hr_double_star = next(a for a in result['attempts'] if a['attempt_raw'] == 'HR**')
