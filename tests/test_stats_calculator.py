import pytest
from softball_statistics.calculators.stats_calculator import calculate_batting_stats, calculate_batting_average, calculate_obp, calculate_slg, calculate_ops
...
class TestStatsCalculator:
    ...
    def test_calculate_obp_with_hbp_and_sf(self):
        ...
        assert calculate_obp(hits=2, walks=1, hbp=1, at_bats=8, sf=1) == pytest.approx(0.364, rel=1e-3)
    ...
    def test_calculate_slg(self):
        ...
        # SLG = 7/4 = 1.750
        assert calculate_slg(singles=2, doubles=1, triples=1, home_runs=0, at_bats=4) == 1.750
    ...
    def test_calculate_slg_with_home_runs(self):
        ...
        # SLG = 9/4 = 2.250
        assert calculate_slg(singles=1, doubles=0, triples=0, home_runs=2, at_bats=4) == 2.250

    def test_calculate_slg_zero_at_bats(self):
        """Test SLG with zero at-bats (should return 0.000)."""
        assert calculate_slg(singles=0, doubles=0, triples=0, home_runs=0, at_bats=0) == 0.000
